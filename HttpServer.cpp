...
static const char* HTML_PAGE =
    ...
    "<script>"
    "var g=1,dbl=0,au=0,atm=0,mw=960,gry=0,rt=2000,_ktm=0;"
    ...
    "if(_KS[e.key]){e.preventDefault();fetch('/api/type?special='+_KS[e.key]);S('key: '+e.key);if(_ktm)clearTimeout(_ktm);_ktm=setTimeout(function(){_ktm=0;R()},2000);return}"
    "if(e.key.length===1){e.preventDefault();fetch('/api/type?char='+encodeURIComponent(e.key));S('key: '+e.key);if(_ktm)clearTimeout(_ktm);_ktm=setTimeout(function(){_ktm=0;R()},2000)}"
    "});"
    /* 加载保存的配置 */
